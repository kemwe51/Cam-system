...
export const modelLayerSchema = z.object({
  id: z.string().min(1),
  label: z.string().min(1),
  kind: z.enum(['source', 'stock', 'features', 'selection', 'operation_preview']),
...
export const derivedGeometryFragmentSchema = z.object({
  ...
  kind: z.enum(['box', 'contour_path', 'pocket_region', 'hole_marker', 'edge_marker', 'text_marker', 'generic_badge']),
...
function normalizeFeatures(part: PartInput): NormalizedFeature[] {
  ...
  part.topSurfaces.forEach((surface, index) => {
    ...
    features.push({ id, sourceId: surface.id ?? id, name: surface.name, kind: 'top_surface', quantity: 1, depthMm: 0, lengthMm: side, widthMm: side, areaMm2: surface.areaMm2, notes: [`Finish target: ${surface.finish}`] });
  });
  part.contours.forEach((contour, index) => {
    const id = featureStableId('contour', index, contour.id);
    features.push({ id, sourceId: contour.id ?? id, name: contour.name, kind: 'contour', quantity: 1, depthMm: contour.depthMm, lengthMm: contour.lengthMm, widthMm: 0, areaMm2: contour.lengthMm * contour.depthMm, notes: ['Derived from structured contour metadata.'] });
  });
  part.pockets.forEach((pocket, index) => {
    const id = featureStableId('pocket', index, pocket.id);
    features.push({ id, sourceId: pocket.id ?? id, name: pocket.name, kind: 'pocket', quantity: 1, depthMm: pocket.depthMm, lengthMm: pocket.lengthMm, widthMm: pocket.widthMm, areaMm2: pocket.lengthMm * pocket.widthMm, notes: ['Derived pocket envelope only.'] });
  });
  part.slots.forEach((slot, index) => {
    const id = featureStableId('slot', index, slot.id);
    features.push({ id, sourceId: slot.id ?? id, name: slot.name, kind: 'slot', quantity: 1, depthMm: slot.depthMm, lengthMm: slot.lengthMm, widthMm: slot.widthMm, areaMm2: slot.lengthMm * slot.widthMm, notes: ['Derived slot envelope only.'] });
  });
  part.holeGroups.forEach((holeGroup, index) => {
    const id = featureStableId('hole-group', index, holeGroup.id);
    features.push({ id, sourceId: holeGroup.id ?? id, name: holeGroup.name, kind: 'hole_group', quantity: holeGroup.count, depthMm: holeGroup.depthMm, lengthMm: holeGroup.diameterMm, widthMm: holeGroup.diameterMm, areaMm2: Math.PI * (holeGroup.diameterMm / 2) ** 2 * holeGroup.count, notes: [`Pattern: ${holeGroup.pattern}`] });
  });
  part.chamfers.forEach((chamfer, index) => {
    const id = featureStableId('chamfer', index, chamfer.id);
    features.push({ id, sourceId: chamfer.id ?? id, name: chamfer.name, kind: 'chamfer', quantity: 1, depthMm: chamfer.sizeMm, lengthMm: chamfer.lengthMm, widthMm: chamfer.sizeMm, areaMm2: chamfer.lengthMm * chamfer.sizeMm, notes: ['Derived chamfer edge request only.'] });
  });
  part.engraving.forEach((engraving, index) => {
    const id = featureStableId('engraving', index, engraving.id);
    features.push({ id, sourceId: engraving.id ?? id, name: engraving.name, kind: 'engraving', quantity: 1, depthMm: engraving.depthMm, lengthMm: engraving.lengthMm, widthMm: 0.2, areaMm2: engraving.lengthMm * 0.2, notes: [`Text: ${engraving.text}`] });
  });
  return features;
}
